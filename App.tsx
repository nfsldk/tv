...
const FILTER_CONFIG: any = {
    movies: { title: '电影', row1: { label: '分类', options: ['全部', '热门电影', '最新电影', '豆瓣高分', '冷门佳片'] }, row2: { label: '类型', options: ['全部', '动作', '喜剧', '爱情', '科幻', '悬疑', '恐怖', '犯罪', '奇幻', '冒险', '灾难', '武侠', '战争', '华语', '欧美', '韩国', '日本'] } },
    series: { title: '电视剧', desc: '来自豆瓣的精选内容', row1: { label: '分类', options: ['全部', '最近热门'] }, row2: { label: '类型', options: ['全部', '国产', '欧美', '日本', '韩国', '动漫', '纪录片', '古装', '武侠', '爱情', '剧情', '悬疑', '喜剧', '家庭', '历史'] } },
    anime: { title: '动漫', desc: '来自 Bangumi 番组计划的精选内容', row1: { label: '分类', options: ['每日放送', '番剧', '剧场版'] }, row2: { label: '类型', options: ['全部', '热血', '恋爱', '搞笑', '校园', '治愈', '机战', '悬疑', '周一', '周二', '周三', '周四', '周五', '周六', '周日'] } },
    variety: { title: '综艺', desc: '来自豆瓣的精选内容', row1: { label: '分类', options: ['全部', '最近热门'] }, row2: { label: '类型', options: ['全部', '大陆', '日本', '韩国', '欧美', '港台'] } }
};

const FilterSection = ({ config, filter1, filter2, onFilter1Change, onFilter2Change }: any) => {
                ...
                <div className="flex items-center gap-3 md:gap-4 mb-4 overflow-x-auto no-scrollbar pb-1">
                    ...
                    <div className="flex gap-2">
                        {config.row1.options.map((opt: string) => (
                            <button key={opt} onClick={() => onFilter1Change(opt)} className={`px-3 md:px-4 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all whitespace-nowrap ${filter1 === opt ? 'bg-gray-600/80 text-white shadow-lg' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'}`}>{opt}</button>
                        ))}
                    </div>
                </div>
                <div className="flex items-center gap-3 md:gap-4 overflow-x-auto no-scrollbar pb-1">
                    ...
                    <div className="flex gap-2">
                        {config.row2.options.map((opt: string) => (
                            <button key={opt} onClick={() => onFilter2Change(opt)} className={`px-3 md:px-4 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all whitespace-nowrap ${filter2 === opt ? 'bg-gray-600/80 text-white shadow-lg' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'}`}>{opt}</button>
                        ))}
                    </div>
                </div>
            </div>
...
const CategoryGrid = ({ category, onItemClick }: { category: string, onItemClick: (item: VodItem) => void }) => {
    ...
    return (
        <div className="animate-fade-in mt-4">
            <FilterSection config={config} filter1={filter1} filter2={filter2} onFilter1Change={setFilter1} onFilter2Change={setFilter2} />
            {loading ? (
                <div className="flex justify-center py-20 min-h-[50vh]"><div className="animate-spin h-10 w-10 border-4 border-brand border-t-transparent rounded-full"></div></div>
            ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-6">
                    {items.map((item) => (
                        <div key={item.vod_id} onClick={() => onItemClick(item)} className="group cursor-pointer relative bg-gray-900 rounded-lg overflow-hidden aspect-[2/3] ring-1 ring-white/5 hover:ring-brand hover:shadow-[0_0_20px_rgba(34,197,94,0.15)] transition-all duration-300 hover:-translate-y-1">
                            <ImageWithFallback src={item.vod_pic || ''} alt={item.vod_name || 'Poster'} searchKeyword={item.vod_name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                            <div className="absolute top-0 right-0 p-1.5 z-10">{item.vod_score && <span className="bg-black/60 backdrop-blur-md text-[10px] text-yellow-400 px-1.5 py-0.5 rounded border border-white/10 shadow-lg font-bold">★ {item.vod_score}</span>}</div>
...
const App: React.FC = () => {
  ...
  const [homeSections, setHomeSections] = useState<{ movies: VodItem[]; series: VodItem[]; shortDrama: VodItem[]; anime: VodItem[]; variety: VodItem[]; }>({ movies: [], series: [], shortDrama: [], anime: [], variety: [] });
  const [heroItems, setHeroItems] = useState<VodItem[]>([]);

  useEffect(() => {
      // SEO Logic here (collapsed for brevity, same as before)
      // ...
  ...
  useEffect(() => {
      ...
      if (path === 'play') {
          ...
          if (id && (!currentMovie || String(currentMovie.vod_id) !== id)) handleSelectMovie(Number(id));
      } else {
  ...
  const handleItemClick = async (item: VodItem) => {
      if (item.source === 'douban') {
          setLoading(true);
          try {
              fetchDoubanData(item.vod_name, item.vod_id); // Prefetch metadata

              // AGGRESSIVE TITLE CLEANING STRATEGY
              // Goal: Get the absolute raw title that matches CMS
              const rawName = item.vod_name;
              
              // 1. Remove Year (e.g. "Name (2023)")
              const noYear = rawName.replace(/[（\(]\d{4}[）\)]/g, '');
              
              // 2. Remove Seasons/Episodes/Versions
              // Matches: Season 1, S01, 第1季, 第12集, 剧场版, 4k
              const cleanName = noYear
                  .replace(/第.+?季|Season\s*\d+|S\d+/gi, '')
                  .replace(/第\s*\d+\s*[集话]/g, '')
                  .replace(/剧场版|电影版/g, '')
                  .replace(/[：:,.，。!！?？\s]/g, ' ') // Punctuation to space
                  .trim();

              // 3. Fallback: First 2 words (e.g. "Rick and Morty" -> "Rick")
              // Useful for very long titles that might mismatch slightly
              const shortName = cleanName.substring(0, 4).trim();

              let foundVideo: VodItem | null = null;
              
              // Waterfall Search: Try Clean Name -> Try Short Name -> Try Original
              const queries = [cleanName, shortName, item.vod_name];
              ...
              for (const q of uniqueQueries) {
                  ...
                  if (res.list && res.list.length > 0) {
                      // Found results! Just take the first one.
              ...
              if (foundVideo) {
                  ...
                  // Last Resort: Go to search page with the CLEANEST name
                  // so the user sees *something* or can adjust easily.
  ...
  const currentEpUrl = useMemo(() => {
      if (currentEpisodeIndex >= 0 && episodes[currentEpisodeIndex]) return episodes[currentEpisodeIndex].url;
      return '';
                  ...
                  <section className="mb-12 animate-fade-in space-y-6 mt-4">
                      ...
                      <div className="flex flex-col lg:flex-row gap-6 items-start h-auto relative transition-all duration-300">
                          <div className={`flex-1 w-full bg-black rounded-xl overflow-hidden border border-white/5 shadow-2xl lg:h-[500px] relative group transition-all duration-300 z-10`}>
                              <VideoPlayer url={currentEpUrl} poster={currentMovie.vod_pic} title={currentMovie.vod_name} episodeIndex={currentEpisodeIndex} doubanId={currentMovie.vod_douban_id} onEnded={() => {if(currentEpisodeIndex < episodes.length - 1) setCurrentEpisodeIndex(prev => prev + 1)}} onNext={() => {if(currentEpisodeIndex < episodes.length - 1) setCurrentEpisodeIndex(prev => prev + 1)}} />
                              {!showSidePanel && (<button onClick={() => setShowSidePanel(true)} className="absolute top-4 right-4 z-20 bg-black/60 backdrop-blur-md text-white/90 border border-white/10 rounded-lg px-3 py-1.5 text-xs flex items-center gap-2 hover:bg-brand/20 hover:border-brand/30 hover:text-brand transition-all shadow-lg opacity-0 group-hover:opacity-100">显示选集</button>)}
                          </div>
                          {showSidePanel && (
                              <div className="w-full lg:w-[350px] bg-[#121212] border border-white/5 rounded-xl overflow-hidden flex flex-col min-h-[150px] max-h-[400px] lg:max-h-none lg:h-[500px] animate-fade-in flex-shrink-0 z-0">
                                  <div className="p-4 border-b border-white/5 bg-gray-800/50 flex justify-between items-center sticky top-0 z-10 backdrop-blur-md">
                                      ...
                                      <button onClick={() => setShowSidePanel(false)} className="text-gray-400 hover:text-white p-1 hover:bg-white/10 rounded transition-colors"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M11.25 4.5l7.5 7.5-7.5 7.5m-6-15l7.5 7.5-7.5 7.5" /></svg></button>
                                  </div>
                                  <div className="p-3 overflow-y-auto custom-scrollbar flex-1 bg-black/20">
                                      <div className="grid grid-cols-4 sm:grid-cols-5 lg:grid-cols-4 gap-2">
                                          {episodes.map((ep) => (<button key={ep.index} onClick={() => setCurrentEpisodeIndex(ep.index)} className={`h-9 text-xs rounded transition-all duration-200 border truncate font-medium ${currentEpisodeIndex === ep.index ? 'bg-brand text-black border-brand shadow-[0_0_10px_rgba(34,197,94,0.3)]' : 'bg-gray-800 text-gray-400 border-transparent hover:bg-gray-700 hover:text-white'}`}>{ep.title.replace(/第|集/g, '')}</button>))}
                                      </div>
                                  </div>
                              </div>
                          )}
              ...
              {!currentMovie && (
                  <div ref={resultsRef}>
                      {(hasSearched || activeTab === 'search') ? (
                        <>
                            <div className="w-full max-w-3xl mx-auto mb-8 mt-2"><form onSubmit={handleSearch} className="relative group"><div className="absolute -inset-0.5 bg-gradient-to-r from-brand to-cyan-500 rounded-full blur opacity-30 group-hover:opacity-60 transition duration-500"></div><div className="relative flex items-center bg-gray-900 rounded-full p-1 ring-1 ring-white/10 shadow-2xl"><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="搜索电影、电视剧、动漫..." className="flex-1 bg-transparent px-6 py-3 text-sm md:text-base text-gray-100 placeholder-gray-500 focus:outline-none" /><button type="submit" disabled={loading} className="bg-brand hover:bg-brand-hover text-black font-bold py-2.5 px-6 rounded-full transition-all duration-300 flex items-center gap-2 shadow-lg disabled:opacity-70">{loading ? '搜索中...' : '搜索'}</button></div></form></div>
                            <div className="flex items-center gap-3 mb-6"><div className="h-6 w-1 bg-brand rounded-full shadow-[0_0_10px_rgba(34,197,94,0.5)]"></div><h3 className="text-xl font-bold text-white tracking-wide">搜索结果</h3></div>
                            {loading && searchResults.length === 0 ? (<div className="flex justify-center py-20"><div className="animate-spin h-10 w-10 border-4 border-brand border-t-transparent rounded-full"></div></div>) : (
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-6">
                                    {searchResults.map((item) => (
                                        <div key={item.vod_id} onClick={() => handleItemClick(item)} className="group cursor-pointer relative bg-gray-900 rounded-lg overflow-hidden aspect-[2/3] ring-1 ring-white/5 hover:ring-brand hover:shadow-[0_0_20px_rgba(34,197,94,0.15)] transition-all duration-300 hover:-translate-y-1">
                                            <ImageWithFallback src={item.vod_pic || ''} alt={item.vod_name || 'Poster'} searchKeyword={item.vod_name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                            <div className="absolute top-0 right-0 p-1.5 z-10"><span className="bg-black/60 backdrop-blur-md text-[10px] text-white px-1.5 py-0.5 rounded border border-white/10 shadow-lg">{item.vod_remarks || '高清'}</span></div>
                                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/80 to-transparent p-2 md:p-3 pt-12"><h4 className="text-xs md:text-sm font-bold text-white truncate group-hover:text-brand transition-colors">{item.vod_name}</h4></div>
                                        </div>
                                    ))}
                                </div>
                            )}
                          ...
                          <div className="space-y-4 animate-fade-in">
                              ...
                              {loading && heroItems.length === 0 && (<div className="flex justify-center py-20"><div className="animate-spin h-10 w-10 border-4 border-brand border-t-transparent rounded-full"></div></div>)}
                              {!loading && (<><HorizontalSection id="movies" title="热门电影" items={homeSections.movies} onItemClick={handleItemClick} /><HorizontalSection id="series" title="热门剧集" items={homeSections.series} onItemClick={handleItemClick} /><HorizontalSection id="short" title="爆款短剧" items={homeSections.shortDrama} onItemClick={handleItemClick} /><HorizontalSection id="anime" title="新番放送" items={homeSections.anime} onItemClick={handleItemClick} /><HorizontalSection id="variety" title="热门综艺" items={homeSections.variety} onItemClick={handleItemClick} /></>)}
                          </div>
