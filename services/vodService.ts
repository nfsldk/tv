...
export const getHomeSections = async (forceRefresh = false) => {
    ...
    const safeFetch = async (fn: Promise<VodItem[]>) => { try { const res = await fn; return Array.isArray(res) ? res : []; } catch (e) { return []; } };
    const [movies, series, shortDrama, anime, variety] = await Promise.all([
        safeFetch(fetchDoubanJson('movie', '热门', 18)), safeFetch(fetchDoubanJson('tv', '热门', 18)),
        safeFetch(fetchDoubanJson('tv', '短剧', 18)), safeFetch(fetchDoubanJson('tv', '日本动画', 18)),
        safeFetch(fetchDoubanJson('tv', '综艺', 18))
    ]);

    const result = { movies, series, shortDrama, anime, variety };
    if (movies.length > 0) {
        try { 
    ...
    const promises = sources.map(async (source) => {
        try {
            ...
            if (data?.list) return data.list.map((item: any) => ({
                ...
                vod_pic: (item.vod_pic?.includes('mac_default') ? '' : item.vod_pic) || '', 
                ...
                source: 'cms', 
                api_url: source.api,
                vod_actor: item.vod_actor, // Preserve metadata for fuzzy sorting
...
/**
 * Improved Aggregated Search with Relevance Scoring
 */
export const getAggregatedSearch = async (keyword: string): Promise<VodItem[]> => {
    ...
    // Merge strategy: Prioritize Douban metadata, but allow unique CMS content
    const allCandidates = [...doubanResults, ...cmsResults];

    // Deduplicate while prioritizing sources (Douban has better metadata/posters)
    allCandidates.forEach(item => {
        const name = item.vod_name.trim();
        if (!seenNames.has(name) || (item.source === 'douban' && finalResults.find(r => r.vod_name === name)?.source !== 'douban')) {
            // If we're replacing a CMS item with a Douban one for better metadata, remove the old one
            if (seenNames.has(name)) {
                const idx = finalResults.findIndex(r => r.vod_name === name);
                if (idx > -1) finalResults.splice(idx, 1);
            }
    ...
    return finalResults.sort((a, b) => {
        ...
        const aActors = (a.vod_actor || '').toLowerCase();
        const bActors = (b.vod_actor || '').toLowerCase();
        const aDirector = (a.vod_director || '').toLowerCase();
        const bDirector = (b.vod_director || '').toLowerCase();

        const getScore = (name: string, actors: string, directors: string) => {
            let score = 0;
            // 1. Exact Name Match (Highest priority)
            if (name === query) score += 1000;
            // 2. Starts with query (High priority)
            else if (name.startsWith(query)) score += 500;
            // 3. Contains query in name (Medium priority)
            else if (name.includes(query)) score += 200;
            
            // 4. Fuzzy Match: Query contained in actors or directors
            // Use a slightly lower weight so title matches always win
            if (actors.includes(query)) score += 100;
            if (directors.includes(query)) score += 50;

            // 5. Douban items get a tiny boost for quality
            if (score > 0 && a.source === 'douban') score += 1;

            return score;
        };

        const scoreA = getScore(aName, aActors, aDirector);
        const scoreB = getScore(bName, bActors, bDirector);

        // Sort descending by score
        if (scoreA !== scoreB) return scoreB - scoreA;
        
        // If scores are equal, sort by year descending
...
export const fetchPersonDetail = async (id: string | number): Promise<PersonDetail | null> => {
    try {
        ...
        const parseWorkLi = (li: Element) => {
            const a = li.querySelector('.pic a'), img = li.querySelector('.pic img'), title = a?.getAttribute('title') || img?.getAttribute('alt') || '', link = a?.getAttribute('href') || '', subjectId = link.match(/subject\/(\d+)/)?.[1];
            if (subjectId && title) return { vod_id: subjectId, vod_name: title, vod_pic: (img?.getAttribute('src') || '').replace(/s_ratio_poster|m(?=\/public)/, 'l'), vod_score: li.querySelector('.rating')?.textContent?.trim() || '', type_name: '影视', source: 'douban', vod_year: '' } as VodItem;
            return null;
        };
        let works: VodItem[] = [];
        doc.querySelectorAll('#best_works .bd ul li, #recent_movies .bd ul li').forEach(li => { const w = parseWorkLi(li); if (w && !works.some(e => e.vod_id === w.vod_id)) works.push(w); });
        return { id: String(id), name: doc.querySelector('#content h1')?.textContent?.trim() || 'Unknown', pic: (doc.querySelector('#headline .pic img')?.getAttribute('src') || '').replace(/s_ratio_poster|m(?=\/public)/, 'l'), gender, constellation, birthdate, birthplace, role, intro: doc.querySelector('#intro .bd')?.textContent?.trim() || '', works };
    } catch (e) { return null; }
};
...
export const parseAllSources = (input: VodDetail | VodDetail[]): PlaySource[] => {
    ...
    details.forEach(detail => {
        ...
        fromArray.forEach((code, idx) => {
            ...
            if (!urlStr || (!code.toLowerCase().includes('m3u8') && !urlStr.toLowerCase().includes('.m3u8'))) return;
            ...
            lines.forEach((line, epIdx) => {
                ...
                if (url?.startsWith('http') || url?.startsWith('//')) episodes.push({ title: (title === code || title.toLowerCase() === 'm3u8' || title.startsWith('http')) ? `第 ${epIdx + 1} 集` : title, url: url.startsWith('//') ? `https:${url}` : url, index: epIdx });
            });
